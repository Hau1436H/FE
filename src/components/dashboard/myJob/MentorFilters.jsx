      ...
      <div className="d-flex gap-3 mb-4">
        ...
        <Dropdown>
          <Dropdown.Toggle 
            ...
            style={{ fontSize: '14px' }}
          >
            {filters.sortBy === 'students' ? 'Học viên đông nhất' : 'Đánh giá cao nhất'}
          </Dropdown.Toggle>
      ...
      <div className="p-4 rounded-4 mb-4" style={{ backgroundColor: '#131520', border: '1px solid #1e2235' }}>
        <div className="row g-4">
          ...
          <div className="col-12 col-md-5">
            <div className="text-white-50 text-uppercase fw-bold mb-3" style={{ fontSize: '11px', letterSpacing: '0.5px' }}>
              ĐÁNH GIÁ TỐI THIỂU: <span style={{ color: '#f59e0b' }}>{(!filters.minRating || filters.minRating === 'all') ? 'TẤT CẢ' : `${filters.minRating}★+`}</span>
            </div>
            <div className="d-flex flex-wrap gap-3 align-items-center">
              {ratings.map(r => {
                // Mặc định chọn 'all' nếu bộ lọc minRating rỗng
                const isSelected = filters.minRating === r.value || (!filters.minRating && r.value === 'all');
                return (
                  <button
