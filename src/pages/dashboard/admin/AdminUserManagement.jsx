...
export default function AdminUserManagement() {
  ...
  const toggleUserStatus = async (user) => {
    try {
      ...
      await axiosClient.put(`/api/admin/users/${user.userId}`, {
        fullName: user.fullName || "",
        roleId: parseInt(user.roleId),
        isActive: newStatus,
      });
      fetchUsers(); // Refresh lại danh sách sau khi đổi
    } catch (err) {
      console.error("Lỗi khi đổi trạng thái:", err);
      alert("Có lỗi xảy ra khi khóa/mở khóa tài khoản.");
    }
  };
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((u) => {
      ...
      const matchesRole =
        roleFilter === "All" || u.roleId.toString() === roleFilter;
      const matchesStatus =
        statusFilter === "All" ||
      ...
      <div className="flex-grow-1 d-flex flex-column overflow-hidden">
        ...
        >
          ...
          >
            {/* TOOLBAR */}
            <div
              className="card-header border-bottom p-3 d-flex flex-wrap align-items-center justify-content-between gap-3"
              style={{
            ...
            >
              ...
              >
                <div
                  ...
                  style={{ left: "10px", color: COLORS.textMuted }}
                >
                  <Search size={16} />
                </div>
                <input
          ...
          <>
            ...
            >
              ...
              <div className="flex-grow-1 overflow-auto p-4">
                <div className="mb-4">
                  <h6
                    className="text-uppercase mb-3 d-flex align-items-center gap-2"
                    style={{ color: COLORS.textMuted, fontSize: "0.75rem" }}
                  >
                    <Lock size={14} /> Account Status
                  </h6>
                ...
                <div className="mb-4">
                  <h6
                    ...
                    style={{ color: COLORS.textMuted, fontSize: "0.75rem" }}
                  >
                    <UserCheck size={14} /> Profile Info
                  </h6>
                  <InputField
                    label="Full Name"
              ...
              <div
                className="p-4 border-top text-end"
                style={{ borderColor: COLORS.border }}
              >
                <button
                  onClick={() => setIsDetailDrawerOpen(false)}
      ...
      <AnimatePresence>
        {isCreateModalOpen && (
          <ModalOverlay onClose={() => setIsCreateModalOpen(false)}>
            ...
            >
              <div
                className="card-header border-bottom p-4 d-flex justify-content-between align-items-center"
                style={{ borderColor: COLORS.border }}
              >
              ...
              <div
                className="card-footer p-4 border-top text-end"
                style={{ borderColor: COLORS.border }}
              >
                <button
                  type="button"
...
const RoleBadge = ({ roleId }) => {
  ...
  return (
    <span
      className={`badge bg-${role.color} bg-opacity-10 text-${role.color} border border-${role.color}`}
    >
      {role.label}
    </span>
  );
...
const InputField = ({ label, ...props }) => (
  <div className="mb-2">
    <label
      ...
      style={{ fontSize: "0.75rem", color: COLORS.textMuted, fontWeight: 500 }}
    >
      {label}
    </label>
    <input
