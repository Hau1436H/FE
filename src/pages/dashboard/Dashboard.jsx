...
const COLORS = { bgContainer: '#000000', accentCyan: '#34D399', textSecondary: '#8C8C8C' };

function Dashboard() {
  const [data, setData] = useState({ overview: null, topGaps: [], topTrends: [] });
    ...
    const fetchAllData = async () => {
      try {
        ...
        rawTrends.forEach(g => {
          ...
          if (points.length > 0) {
            const sorted = [...points].sort((a, b) => new Date(b.analyzedDate || b.AnalyzedDate) - new Date(a.analyzedDate || a.AnalyzedDate));
            latestDemand[name] = sorted[0].demandPercent || sorted[0].DemandPercent || 0;
          }
        });
        ...
        setError("Chưa thiết lập mục tiêu nghề nghiệp. Vui lòng cập nhật trong Hồ sơ của bạn.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const connection = new HubConnectionBuilder()
      .withUrl("https://localhost:7196/hubs/roadmap", { accessTokenFactory: () => token })
  ...
  return (
    <div className="d-flex" style={{ backgroundColor: COLORS.bgContainer, minHeight: '100vh', fontFamily: 'system-ui' }}>
      <Sidebar />

      <div className="flex-grow-1 p-4 overflow-auto text-white" style={{ maxHeight: '100vh', backgroundColor: '#06060c' }}>
        <DashboardHeader />

        {loading ? (
          <div className="text-center py-5 mt-5">
            <div className="spinner-border mb-3" style={{ color: COLORS.accentCyan }}></div>
            <p style={{ color: COLORS.textSecondary }}>AI đang tổng hợp chiến lược học tập cho bạn...</p>
          </div>
        ) : error || !overview ? (
          <div className="alert mt-4 text-center py-4 rounded-4" style={{ backgroundColor: 'rgba(220,53,69,0.1)', border: '1px solid rgba(220,53,69,0.3)', color: '#E5635B' }}>
